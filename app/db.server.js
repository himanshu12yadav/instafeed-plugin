...
export const getAllInstagramAccounts = async (shop) => {
...
export const getAccountDeletionDetails = async (accountId) => {
  ...
  const totalProductLinks = posts.reduce(
    (sum, post) => sum + post.products.length,
    0
  );
...
export const storeInstagramPosts = async (posts = [], accountId, shop) => {
  try {

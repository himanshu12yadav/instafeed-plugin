...
export const loader = async ({ request }) => {
  ...
  const BYPASS_SUBSCRIPTION =
    process.env.BYPASS_SUBSCRIPTION === 'true' || false;
  ...
  if (BYPASS_SUBSCRIPTION) {
    ...
    const accounts = await getAllInstagramAccounts(shop);
...
function shouldNotifyTokenExpiring(expireDate, REFRESH_BUFFER_MS) {
  const now = Date.now();
  const expireTime = expireDate.getTime();
  const notifyTime = expireTime - REFRESH_BUFFER_MS;
  ...
  if (now >= notifyTime) {
    return { remaining: expireTime - now, expired: false };
  }
...
export const action = async ({ request }) => {
  ...
  const selectedAccount = formData.get('selectedAccount')
    ? JSON.parse(formData.get('selectedAccount'))
    : null;



  // getting checked post
  const checkedPost = formData.get('checkedPost')
    ? JSON.parse(formData.get('checkedPost'))
    : null;

  // refresh the posts
  const refreshInstagramPosts = formData.get('refreshInstagramPosts')
    ? JSON.parse(formData.get('refreshInstagramPosts'))
    : null;

  // search query
  const searchQuery = formData.get('searchQuery')
    ? JSON.parse(formData.get('searchQuery'))
    : null;
  ...
  if (refreshInstagramPosts) {
    ...
    const {
      ...
      id,
      instagramTokenExpires: expires,
    } = await findUserByInstagramUsername(selectedAccount);

    if (!expires) {
      message['error'].push({
        error: 'Unable to find expired token.',
      });
    ...
    const isExpiredResponse = shouldNotifyTokenExpiring(
      expires,
      REFRESH_BUFFER_MS
    );

    if (isExpiredResponse.expired) {
      message['error'].push({
        error: 'Token expired.',
      });
    ...
    if (!isExpiredResponse.expired) {
      message['error'].push({
        success: `${isExpiredResponse.remaining} remaining.`,
      });
    }
    ...
    if (refresh) {
      ...
      await storeInstagramPosts(currentPosts, id, shop);
  ...
  if (checkedPost && Object.keys(checkedPost).length > 0) {
    ...
    if (postId && currentPost.id) {
      const { accountId } = currentPost;

      await updatePostData(currentPost.id, 'selected', selectionStatus);
      const posts = await getAllInstagramPostbyAccountId(accountId);
      ...
      return {
        data: await getAllInstagramPostbyAccountId(accountId),
        captionLists,
        message,
      };
    }
  ...
  if (selectedAccount && Object.keys(selectedAccount).length > 0) {
    ...
    const expires = dbUsername?.instagramTokenExpires;

    if (!dbUsername) {
      message['success'].push({
        success: 'Please select username.',
      });
      return {
        data: [],
        message,
    ...
    if (!isExpiredResponse.expired) {
      message['success'].push({
        success: `${new Date(Date.now() + isExpiredResponse.remaining)} remaining.`,
      });
    }
    ...
    await storeInstagramPosts(posts, dbUsername.id, shop);

    message['success'].push({
      success: 'Instagram post fetched successfully.',
    });

    return {
      data: await getAllInstagramPostbyAccountId(dbUsername.id),
      message,
    };
  }
...
export default function Index() {
  ...
  useEffect(() => {
    ...
    if (actionData.message?.success?.length > 0) {
      actionData.message?.success?.map(({ success }, index) =>
        setTimeout(() => setToaster(success), index * 1500)
      );
  ...
  const instagramUrl = `https://www.instagram.com/oauth/authorize?force_reauth=true&client_id=624455150004028&redirect_uri=https://instagramfeed-app.sprinix.com/auth/instagram/callback&response_type=code&scope=instagram_business_basic&state=${state}`;
    ...
    if (actionData?.data && actionData.data.length > 0) {
      ...
      if (firstPost.account) {
        ...
        const foundAccount = accounts?.find(
          (account) => account.instagramUsername === selected
        );
  ...
  const fetchProductCounts = async (accountId) => {
    try {
      ...
      let response = await fetch(
        `/api/products?action=getCounts&accountId=${accountId}`
      );
      let data = await response.json();
  ...
  return hasActiveSubscription ? (
    <Frame>
      ...
      >
        {toaster ? (
          <Toast content={toaster} onDismiss={toastActive} duration={2000} />
        ) : null}
        <Layout>
          <Layout.Section>
            <Card sectioned>
                ...
                <Grid>
                  ...
                  {!isLoading &&
                    userData?.map((post) => (
                      <Grid.Cell
                        key={post.id}
                        columnSpan={{ xs: 6, sm: 6, md: 3, lg: 3 }}
                      >
                        <Card>
                              ...
                              <InlineStack gap="100" align="start">
                                ...
                                {productCounts[post.id] > 0 && (
                                  <Badge tone="success" size="small">
                                    🛍️ {productCounts[post.id]} Product
                                    {productCounts[post.id] !== 1 ? 's' : ''}
                                  </Badge>
                                )}
                              </InlineStack>
                            </BlockStack>
            ...
            <FooterHelp>
              <BlockStack gap="400" alignment={'center'}>
                <Text variant={'bodySm'} as={'p'} color={'subdued'}>
                  © 2024 Instafeed-sprinix. All rights reserved.
                </Text>
              </BlockStack>
            </FooterHelp>
          </Layout.Section>
              ...
              <Box padding={'400'}>
                <BlockStack gap="500">
                  <BlockStack gap="200">
                    ...
                    <Text as="p" variant="bodyMd" color="subdued">
                      Search and select a product from your store to link with
                      this Instagram post. This will help customers discover
                      your products directly from your Instagram feed.
                    </Text>
                  </BlockStack>
                              ...
                              return (
                                <div
                                  ...
                                  onClick={() => {
                                    if (isAlreadyLinked) {
                                      setToaster(
                                        'This product is already linked to this post!'
                                      );
                                      return;
                                    }
                  ...
                  <InlineStack gap="300" align="end">
                    <Button onClick={handleProductModalClose}>Cancel</Button>
                    <Button
                      primary
                      onClick={async () => {
                        if (selectedProduct) {
              ...
              <Box padding={'400'}>
                <BlockStack gap="400" alignment="center">
                  ...
                  <Text as="p" variant="bodyMd" alignment="center">
                    Thank you for installing our Instagram feed app! Here's what
                    you can do:
                  </Text>
                  <List type="bullet">
                    <List.Item>
                    ...
                    <List.Item>
                      Display Instagram content on your storefront
                    </List.Item>
                  </List>
                  <Text as="p" variant="bodyMd" alignment="center">
          ...
          <Layout.Section>
            <Card sectioned>
              <BlockStack gap={'400'}>
                ...
                <List type="bullet">
                  <List.Item>
                    Unlimited access to Instagram feed features.
                  </List.Item>

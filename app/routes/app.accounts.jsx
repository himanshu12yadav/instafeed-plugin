...
export default function AccountsPage() {
  ...
  const handleDeleteClick = async (account) => {
    ...
    try {
      const response = await fetch(
        `/app/accounts?accountId=${account.id}&action=getDeletionDetails`
      );
  ...
  const rows = accounts.map((account) => [
    ...
    <Badge
      tone={account.accountType === 'BUSINESS' ? 'success' : 'info'}
      size="small"
    >
  ...
  return (
    <Frame>
      ...
      >
        {toaster && (
          <Toast content={toaster} onDismiss={toastActive} duration={3000} />
        )}
                ...
                <InlineStack align="space-between">
                  <BlockStack gap="200">
                    ...
                    <Text as="p" variant="bodyMd" color="subdued">
                      View and manage all your connected Instagram business
                      accounts.
                    </Text>
                  </BlockStack>
            ...
            <BlockStack gap="400">
              ...
              <ul style={{ paddingLeft: '20px' }}>
                ...
                <li>
                  <Text as="p" variant="bodyMd">
                    All{' '}
                    {deletionDetails?.postsCount ||
                      accountToDelete?.posts?.length ||
                      0}{' '}
                    associated Instagram posts
                  </Text>
                </li>
                {deletionDetails?.selectedPostsCount > 0 && (
                  <li>
                    <Text as="p" variant="bodyMd">
                      {deletionDetails.selectedPostsCount} selected posts that
                      are currently displayed on your storefront
                    </Text>
                  </li>
                )}
                {deletionDetails?.productLinksCount > 0 && (
                  <li>
                    <Text as="p" variant="bodyMd">
                      {deletionDetails.productLinksCount} product links
                      associated with these posts
                    </Text>
                  </li>
                )}
                {(!deletionDetails?.productLinksCount ||
                  deletionDetails.productLinksCount === 0) && (
                  <li>
                    <Text as="p" variant="bodyMd">

...
export const loader = async ({ request }) => {
  ...
  if (instefeedData) {
    ...
    if (isSubscribe || process.env.BYPASS_SUBSCRIPTION === 'true') {
      return await handleInstagramSelectedData(shop);
    }
...
const handleInstagramSelectedData = async (shop) => {
  ...
  const postsWithProducts = await prisma.instagramPost.findMany({
    ...
    include: {
      products: true,
      account: true, // Include the product relationships
    },
    orderBy: {
      timestamp: 'desc',
    },
  ...
  const transformedPosts = postsWithProducts
    .map((post) => ({
      ...post,
    ...
    .filter((post) => post.account.shop === shopSubdomain);

...
export const loader = async ({ request }) => {
  ...
  const { shop } = JSON.parse(atob(state));
  ...
  try {
    ...
    const requestData = qs.stringify({
      ...
      redirect_uri:
        'https://instagramfeed-app.sprinix.com/auth/instagram/callback',
      code,
    });
    ...
    const instagramTokenExpires = new Date(
    ...
    const data = {
      ...
      instagramTokenExpires: instagramTokenExpires, // 30 days from now,
    ...
    if (Object.keys(result).length > 0) {
      return redirect(
        `https://admin.shopify.com/store/${shop}/apps/instafeed-sprinix/app`,
        {
          status: 302,
        }
      );
